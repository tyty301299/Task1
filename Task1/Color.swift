

import Foundation
import UIKit
struct Color {
    static let backgroundColor = UIColor(red: 0 / 255, green: 0 / 255, blue: 0 / 255, alpha: 1)
    static let buttonColorNum = UIColor(red: 52 / 255, green: 52 / 255, blue: 52 / 255, alpha: 1)
    static let buttonAnswer = UIColor(red: 242 / 255, green: 163 / 255, blue: 60 / 255, alpha: 1)
    static let buttonColorAC = UIColor(red: 165 / 255, green: 165 / 255, blue: 165 / 255, alpha: 1)
    static let title = UIColor.white
    static let titleAc = UIColor.black
}
